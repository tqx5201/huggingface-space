...
def get_m3u8(video_id: str):
    ...
    for i in range(3):
        playlist += "#EXTINF:4,\n"
        playlist += f"/id/{video_id}/ts/{timestamp}.ts\n"
        timestamp += 1
        t += 4
...
def get_ts(video_id: str,ts:str):
    if video_id not in CHANNEL_MAPPING:
        ...
        return None
    t = int(ts)*4
    url = f"https://ntd-tgc.cdn.hinet.net/live/pool/{video_id}/litv-pc/{video_id}-avc1_6000000={CHANNEL_MAPPING[video_id][0]}-mp4a_134000_zho={CHANNEL_MAPPING[video_id][1]}-begin={t}0000000-dur=40000000-seq={ts}.ts"
    #return url
